...
public class ObjectifController {
	...
	public Object getAllObjectif()
	{
		...
		Type listType = new TypeToken<List<ObjectifDto>>() {}.getType();

		List<ObjectifDto> listobjectifdtos = modelMapper.map(listobjectif,listType);
		return ResponseEntity.status(HttpStatus.OK).body(listobjectifdtos);
	}
	...
	public Object getObjectifById(@PathVariable("idO") Long idO)
    {
		...
		ObjectifDto objectifDto=modelMapper.map(objectif,ObjectifDto.class);
		return ResponseEntity.status(HttpStatus.OK).body(objectifDto);
    }
